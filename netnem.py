#!/usr/bin/env python3
"""
GhostMesh CLI Tool v1.4 (Clean Termux + Group Fix)
...
def initial_setup():
    ...
    if "android" in os_name or (os_name == "linux" and os.path.isdir("/data/data/com.termux")):
        ...
        print("[✔] Termux dependencies installed.\n")
    else:
    ...
    if subprocess.call(checker, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL) != 0:
        print("⚠ Git not found. Install it manually.")
    else:
...
def group_acceptor(local_recv_handler):
    srv = socket.socket()
    srv.setsockopt(socket.SOL_SOCKET, socket.SO_REUSEADDR, 1)
    srv.bind(("0.0.0.0", PORT))
    srv.listen()
    print(f"[Group] Hosting on port {PORT}…")
    threading.Thread(target=local_recv_handler, daemon=True).start()
    while True:
        conn, addr = srv.accept()
        with group_lock:
...
if __name__ == "__main__":
    ...
    if mode == "ptp":
        ...
        if role == 'h':
                ...
                while True:
                    try:
                        ...
                        if msg == "/exit":
                            ...
                            fn = msg.split(maxsplit=1)[1] if " " in msg else "chat.fasta"
                            if not os.path.isfile(fn): print("Not found:", fn); continue
                            lines = [l.strip() for l in open(fn) if l.strip()]
                            for i in range(0, len(lines), 2):
                                u, sd = lines[i][1:].rsplit("_seed_", 1)
            ...
            threading.Thread(target=group_acceptor, args=(host_echo,), daemon=True).start()
