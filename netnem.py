#!/usr/bin/env python3
"""
GhostMesh CLI Tool v1.3
Modes: PtP (peer-to-peer) or Group (multi-client broadcast)
Features:
- Auto-dependency on Android, Linux, Windows, macOS
...
def initial_setup():
    ...
    if "android" in os_name or (os_name == "linux" and os.path.isdir("/data/data/com.termux")):
        ...
        os.system("chmod +x netnem.py")
    # Git on others
    if os_name in ("linux","darwin","windows"):
        check = ["where","git"] if "windows" in os_name else ["which","git"]
        if subprocess.call(check, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)!=0:
            if "linux" in os_name: os.system("sudo apt update && sudo apt install git -y")
            elif "darwin" in os_name: os.system("brew install git")
            else: print("Install Git: https://git-scm.com/download/win")
...
def show_ipv4():
    ...
    try:
        ...
        print("Local IPv4:")
        for ip in sorted(set(ips)): print(" -", ip)
    except:
        print("IPv4 scan failed.")
...
def gen_map(seed):
    random.seed(seed)
    b=['00','01','10','11']; nts=NUCS.copy(); random.shuffle(nts)
    return {b[i]: nts[i] for i in range(4)}
...
def group_acceptor():
    ...
    while True:
        ...
        threading.Thread(target=group_receiver, args=(conn,)).start()
...
def group_join(ip):
    ...
    threading.Thread(target=group_receiver, args=(s,)).start()
    return s
...
if __name__=="__main__":
    ...
    if mode=="ptp":
        role=input("Server or Client? [s/c]: ").lower()
        conn = ptp_server() if role=="s" else ptp_client(input("Server IP: ").strip())
    else:
        role=input("Host or Join? [h/j]: ").lower()
        if role=="h":
    ...
    while True:
        ...
        if msg.startswith("/load"):
            fn=msg.split(maxsplit=1)[1] if " " in msg else "chat.fasta"
            if not os.path.exists(fn): print("No",fn); continue
